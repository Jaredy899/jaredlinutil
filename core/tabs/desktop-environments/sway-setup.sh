...
installSway() {
    ...
    case "$PACKAGE_MANAGER" in
        apt-get|nala)
            ...
            "$ESCALATION_TOOL" "$PACKAGE_MANAGER" install -y sway swaylock swayidle waybar
            installDisplayManager
            ;;
        dnf)
            "$ESCALATION_TOOL" "$PACKAGE_MANAGER" install -y sway swaylock swayidle waybar
        ...
        pacman)
            ...
            "$ESCALATION_TOOL" "$PACKAGE_MANAGER" -S --noconfirm sway swaylock swayidle waybar
            installDisplayManager
            ;;
        zypper)
        ...
        apk)
            "$ESCALATION_TOOL" "$PACKAGE_MANAGER" update
            "$ESCALATION_TOOL" "$PACKAGE_MANAGER" add sway swaylock swayidle waybar
            installDisplayManager
            ;;
        xbps-install)
            "$ESCALATION_TOOL" "$PACKAGE_MANAGER" -Su
            "$ESCALATION_TOOL" "$PACKAGE_MANAGER" -y sway swaylock swayidle waybar
            installDisplayManager
            ;;
        slapt-get)
            "$ESCALATION_TOOL" "$PACKAGE_MANAGER" --update
            "$ESCALATION_TOOL" "$PACKAGE_MANAGER" --install sway swaylock swayidle waybar
            installDisplayManager
            ;;
        *)
